    ...
    def load_data_to_tensors(self,path):
        ...
        for index, row in DataFrame.iterrows():
            ...
            if self.Nlabel > 1:
                ...
                #if self.forget:
                #    pict_label += (np.random.random(size = pict_label.shape) - 0.5)*50
...
def train(agent, epochs = 1, batch_size = 128, \
    ...
    if not isINN:
        ...
        name_for_save = path_to_model_for_save
        if iskan: name_for_save += "_kan"

        torch.save(model, name_for_save)

        for el in [0, 1, 10, 100, 200, 500, 700, 900, 2000, 5000, 10000, 50000,75000]:
